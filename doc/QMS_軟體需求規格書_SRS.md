...
- **FR-3.1 批次放行**：背景 Worker 根據後端系統的處理量能，定時從 Redis `ZSET` 取出特定數量的使用者名單，並更新已放行最大號碼。
- **FR-3.2 授權事件發布**：系統將放行名單封裝為授權事件送入 Kafka。
- **FR-3.3 業務處理與通知**：核心微服務消費 Kafka 事件進行處理，完成後透過 SSE 通知前端跳轉至結帳或劃位區。
